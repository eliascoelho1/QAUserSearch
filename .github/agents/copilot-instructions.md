...
- PostgreSQL (catálogo de schemas via SQLAlchemy async), arquivos JSON (amostras MOCK em `res/db/`) (001-external-schema-extraction)
- Python 3.11 + FastAPI, CrewAI, OpenAI SDK, SQLAlchemy (async), Pydantic (001-llm-query-interpreter)
- PostgreSQL (via asyncpg) para dados de QA, logs de auditoria em banco existente (001-llm-query-interpreter)
...
## Recent Changes
- 001-llm-query-interpreter: Added Python 3.11 + FastAPI, CrewAI, OpenAI SDK, SQLAlchemy (async), Pydantic
